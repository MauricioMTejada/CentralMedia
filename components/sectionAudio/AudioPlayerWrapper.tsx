...
const AudioPlayerWrapper = () => {
	...
	const handleWheel = (e: WheelEvent) => {
		if (audioPlayerRef.current?.audio.current) {
			const currentVolume = audioPlayerRef.current.audio.current.volume || 0;

            // el siguiente número decimal define el paso de la rueda del ratón para cambiar el volumen
			const newVolume = Math.min(Math.max(0, currentVolume - e.deltaY * 0.0005), 1);
